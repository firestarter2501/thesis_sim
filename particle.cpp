...
void particle::move_test(double dist)
{
    this->initptcl(0, 0, 0, 0);
    this->move(dist);
    std::cout << "x: " << dist * sin(this->dir_theta_) * cos(this->dir_phi_) << "\ty: " << dist * sin(this->dir_theta_) * sin(this->dir_phi_) << "\tz: " << dist * cos(this->dir_theta_) << "\n" << "dist: " << sqrt(pow(dist * sin(this->dir_theta_) * cos(this->dir_phi_), 2) + pow(dist * sin(this->dir_theta_) * sin(this->dir_phi_), 2) + pow(dist * cos(this->dir_theta_), 2)) << std::endl;
}
...
void particle::turn_test(double angle)
{
    ...
    std::cout << "theta: " << this->dir_theta_ << "\tphi: " << this->dir_phi_ << "\nangle: " << acos(((before_x * after_x) + (before_y * after_y) + (before_z * after_z)) / (sqrt(pow(before_x, 2) + pow(before_y, 2) + pow(before_z, 2)) * sqrt(pow(after_x, 2) + pow(after_y, 2) + pow(after_z, 2)))) << std::endl;
}